    ...
    postForm.addEventListener('submit', (e) => {
        try {
            ...
            if (!contentField || contentField.value.trim() === '') {
                e.preventDefault();
                alert(gettext('Помилка: Пост не може бути порожнім! Заповніть хоча б один текстовий блок.'));
                return;
            }
            ...
            e.preventDefault();
            alert(gettext('Сталася внутрішня помилка конструктора. Перевірте консоль браузера.'));
        }
    });
...
window.formatText = function(btn, type) {
    ...
    switch (type) {
        case 'h1':
            prefix = '# '; suffix = ''; placeholder = gettext('Головний заголовок');
            if (start > 0 && text[start - 1] !== '\n') prefix = '\n# ';
            break;
        case 'h2':
            prefix = '## '; suffix = ''; placeholder = gettext('Заголовок 2 рівня');
            if (start > 0 && text[start - 1] !== '\n') prefix = '\n## ';
            break;
        case 'h3':
            prefix = '### '; suffix = ''; placeholder = gettext('Заголовок 3 рівня');
            if (start > 0 && text[start - 1] !== '\n') prefix = '\n### ';
            break;
        case 'bold':
            prefix = '**'; suffix = '**'; placeholder = gettext('жирний текст');
            break;
        case 'italic':
            prefix = '*'; suffix = '*'; placeholder = gettext('курсив');
            break;
        case 'link':
            const url = prompt(gettext('Введіть посилання (URL):'), 'https://');
            if (!url) return;
            prefix = '['; suffix = `](${url})`; placeholder = gettext('текст посилання');
            break;
        case 'ul':
            prefix = '- '; suffix = ''; placeholder = gettext('пункт списку');
            if (start > 0 && text[start - 1] !== '\n') prefix = '\n- ';
            break;
        case 'ol':
            prefix = '1. '; suffix = ''; placeholder = gettext('перший пункт');
            if (start > 0 && text[start - 1] !== '\n') prefix = '\n1. ';
            break;
    }
...
window.previewImage = function(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            label.querySelector('strong').textContent = '🔄 ' + gettext('Змінити фото');
        }
        reader.readAsDataURL(input.files[0]);
    } else {
        container.style.display = 'none';
        label.querySelector('strong').textContent = gettext('Натисніть, щоб обрати фото');
    }
};
...
window.addBlock = function(type, content = '', lang = '') {
    ...
    const badgeText = type === 'text' ? gettext('Текст') : type === 'code' ? gettext('Фрагмент коду') : gettext('Фотографія');

    let html = `
        <div class="block-controls">
            <span class="block-type-badge">${badgeText}</span>
            <div class="block-actions">
                <button type="button" onclick="moveBlock(this, -1)" title="${gettext('Вгору')}">▲</button>
                <button type="button" onclick="moveBlock(this, 1)" title="${gettext('Вниз')}">▼</button>
                <button type="button" class="delete-btn" onclick="this.closest('.editor-block').remove()" title="${gettext('Видалити')}">✖</button>
            </div>
        </div>
    `;
    ...
    if (type === 'text') {
        html += `
            <div class="text-format-toolbar">
                <button type="button" class="format-btn" onclick="formatText(this, 'h1')" title="${gettext('Заголовок 1')}">H1</button>
                <button type="button" class="format-btn" onclick="formatText(this, 'h2')" title="${gettext('Заголовок 2')}">H2</button>
                <button type="button" class="format-btn" onclick="formatText(this, 'h3')" title="${gettext('Заголовок 3')}">H3</button>
                <span style="border-left: 1px solid var(--border); margin: 0 4px;"></span>
                <button type="button" class="format-btn" onclick="formatText(this, 'bold')" title="${gettext('Жирний')}"><b>B</b></button>
                <button type="button" class="format-btn" onclick="formatText(this, 'italic')" title="${gettext('Курсив')}"><i>I</i></button>
                <button type="button" class="format-btn" onclick="formatText(this, 'link')" title="${gettext('Посилання')}">🔗</button>
                <button type="button" class="format-btn" onclick="formatText(this, 'ul')" title="${gettext('Маркований список')}">• ${gettext('Список')}</button>
                <button type="button" class="format-btn" onclick="formatText(this, 'ol')" title="${gettext('Нумерований список')}">1. ${gettext('Список')}</button>
            </div>
            <textarea class="form-control block-data" rows="4" placeholder="${gettext('Почніть писати тут...')}">${content}</textarea>
        `;
    } 
    else if (type === 'code') {
        html += `
            <input type="text" class="form-control block-lang" placeholder="${gettext('Мова (напр. python, js, html)')}" value="${lang}" style="margin-bottom: 10px; width: 200px;">
            <textarea class="form-control block-data" rows="5" placeholder="${gettext('Вставте ваш код...')}">${content}</textarea>
        `;
    } 
    else if (type === 'image') {
        let previewSrc = '';
        ...
        if (content) {
            ...
            if (urlMatch) {
                ...
                imageInfoMsg = `<span style="color: var(--royal-purple); font-size: 0.85rem; display:block; margin-top: 10px;">${gettext('Збережене фото. Натисніть кнопку вище, щоб замінити його.')}</span>`;
            }
        }

        const safeContent = content.replace(/'/g, "&apos;").replace(/"/g, "&quot;");
        const btnText = content ? '🔄 ' + gettext('Змінити фото') : gettext('Натисніть, щоб обрати фото');
        ...
        html += `
            <div style="padding: 15px; text-align: center; border: 2px dashed var(--royal-purple); border-radius: 8px; background: rgba(120, 81, 169, 0.05);">
                <label style="cursor: pointer; display: block; margin: 0;">
                    ...
                    <strong>${btnText}</strong><br>
                    <span style="color: var(--text-muted); font-size: 0.8rem;">${gettext('(Файл буде завантажено автоматично)')}</span>
...
window.togglePreview = function() {
    ...
    if (previewContainer.style.display === 'none') {
        ...
        previewContainer.innerHTML = marked.parse(rawContent) || `<p style="color: var(--text-muted); text-align: center;">${gettext('Пост порожній. Додайте контент для перегляду.')}</p>`;
        
        editorBlocks.style.display = 'none';
        addMenu.style.display = 'none';
        previewContainer.style.display = 'block';
        
        previewBtn.innerHTML = '✏️ ' + gettext('Повернутися до редагування');
        ...
        previewBtn.innerHTML = '👁️ ' + gettext('Попередній перегляд');
